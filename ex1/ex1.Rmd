...
```
Next, we need to check if this gives reasonable results by comparing our simulated values to the theoretical knowledge using $\lambda = 0.5$ and $n = 1000$. The result is shown in figure 1.
```{r, cache=TRUE, fig.cap= "The theoretical density of the exponential distribution plotted with the distribution of our generetad samples." }
rate <- 0.5
n <- 10000
...
```
We also compare the estimated mean and variance to first and second central moments of the exponential distribution. The results are shown in figure 2 and 3.

```{r, fig.cap="Theoretical mean plotted against sample mean for the exponential distribtion."}
...
```{r, fig.cap="Theoretical variance plotted against sample mean for the exponential distribtion."}
...
-\ln\left(\dfrac{1-u}{c}\right), & \dfrac{c}{\alpha} \le u \le 1.
\end{cases}
$$

### (b)
First we implement $g$ and an algorithm to generate samples from $g$:
```{r}
g <- function(alpha, x){
...
To test our implementation we use $\alpha = 0.5$ and simulate $n=10000$ samples. We plot the distribution of the samples with the exact density in figure 4.

```{r, fig.cap="The theoretical density of g plotted with the distribution of our generetad samples."}
alpha <- 0.5
n <- 100000
sim <- data.frame(x = sim.g(alpha, n))
x = seq(from = 0.01, to = 4, by = 0.05)
...
```
From the histogram, we see that we are able to generate samples from $g$ corresponding to the true density. Next we find the theoretical mean and variance and compare with the estimated values. Let $X \sim g(x)$. The theoretical mean is then given by
$$
\mathrm{E}[X] = \int_{-\infty}^{\infty} x\ g(x)\ \text{d}x = c \int_{0}^1 x^{\alpha}\ \text{d}x + c \int_{1}^{\infty} x e^{-x}\ \text{d}x = \frac{c}{\alpha+1} + \frac{2c}{e}.
$$
Similarly, we find the theoretical variance to be
$$
\begin{split}
\text{Var}[X] & = \int_{-\infty}^{\infty} x^2 f(x)\ \text{d}x - \mu_g^2 \\
...
```{r}
sim.sigm <- function(n, alpha){
  u <- runif(n,0,1)
  1/alpha * (log(u) - log(1-u))
}
```
We compare the simulated values with the theoretical distribution. This is done graphically in figure 5.
```{r, cache = TRUE, fig.cap="The theoretical density of f plotted with the distribution of our generetad samples."}
alpha <- 2
n <- 1000000
sim <- data.frame(x = sim.sigm(n, alpha))
...
To compare the simulated values with the theoretical mean and variance, we first need to compute the expression of these moments. Let $X \sim f(x)$. Then 

$$
\mathrm{E}[X] = \int_{-\infty}^\infty \frac{\alpha xe^{\alpha x}}{(1 + e^{\alpha x})^2}dx = 0,
$$
because the integrand is an odd function. The sample mean of our simulation is computed below
```{r}
mean(sim$x)
```
We see that the sample mean is approximately zero. Next, the theoretical variance is given as

$$
\mathrm{Var}[X] = \mathrm{E}[X^2] = \int_{-\infty}^\infty \frac{\alpha x^2e^{\alpha x}}{(1 + e^{\alpha x})^2}dx,
$$
for which we were not able to find a closed form. Therefore, we consider the variance when $\alpha = 2$. Then, $\mathrm{Var}[X] = \frac{\pi^2}{12} \approx 0.8225$, and the sample variance is computed to be
```{r}
var(sim$x)
```
which showcases that the simulation algorithm works as intended.
...
```
As usual, we compare the results of a simulation against the theoretical distribution. This is done graphically in figure 6.
```{r, cache=TRUE, fig.cap= "The theoretical density of the standard normal distribution plotted with the distribution of our generetad samples."}
n <- 10000
...
ggplot(sim.box.mul) + 
  ...
  scale_fill_manual(name = "", values = c("Simulated" = "#69b3a2"))
```
Finally, we compare the mean and variance resulting from the simulation with the theoretical values in figure 7 and 8, respectively. 
```{r, fig.cap="Theoretical mean plotted against sample mean for the standard normal distribution."}
# Mean
data.frame(n.samples = 1:n,
...
```{r, fig.cap="Theoretical variance plotted against sample variance for the standard normal distribution."}
# Variance
data.frame(n.samples = 1:n,
           mean = cumsum(sim.box.mul$x)/(1:n),
...
To test our implementation, we generate a bivariate normal distribution and plot some of the samples with the theoretical level curves. This can be viewed in figure 9.
```{r, fig.cap="Simulated samples from the bivariate normal distribution plotted with the theoretical level curves."}
# Bivariate example
mu <- c(1,2)
...
```
We conduct our usual sanity check for $\alpha = 0.5$ and $n = 10000$ samples, shown in figure 10.
```{r, cache=TRUE, fig.cap="The theoretical density of the gamma distribution plotted with the distribution of our samples generated with rejection sampling."}
n <- 10000
alpha <- 0.5
...
We also plot the sample mean and variance against the theoretical values, shown in figure 11 og 12.
```{r, fig.cap="Theoretical mean plotted against sample mean for the gamma distribution."}
# Mean
data.frame(n.samples = 1:n,
...
```{r, fig.cap="Theoretical variance plotted against sample variance for the gamma distribution."}
# Variance
data.frame(n.samples = 1:n,
           mean = cumsum(sim.gamma.rej$x)/(1:n),
...
The algorithm which uses the ratio of uniforms-method to sample from the gamma distribution with $\alpha >0$ and $\beta = 1$ is given below. Note that the algorithm is implemented on the log scale in order to avoid numerical overflow. Hence, we sample from $Y = \ln X$, where $X\sim \mathcal{U}_{[a,b]}$, in stead of sampling directly from $X$. To achieve this, we have first found the CDF of $Y$ and then used the inversion method.

```{r}
# Function to sample from log(Uniform)
...
# Function for the log of the core of the gamma distribution.
...
First, we check that the simulation algorithm gives reasonable results, by plotting the samples and the exact density in figure 13 with $\alpha =10$ $n=100000$ samples.
```{r, cache=TRUE, fig.cap="The theoretical density of the gamma distribution plotted with the distribution of our generetad samples with ratio of uniforms method."}
n <- 100000
alpha <- 10
...
```
The algorithm seems to yield reasonable results. We also compare the simulation with theoretical mean and variance of the distribution.
```{r}
c("mean.theoretical" = alpha, "mean.sample" = mean(sim.gamma$sim))
c("var.theoretical" = alpha, "var.sample" = var(sim.gamma$sim))
```

Judging from the printout above, we conclude that the ratio of uniforms algorithm works correctly. Next we plot the necessary tries to sample $n=1000$ samples with this algorithm for $\alpha \in (0, 2000]$ in figure 14.

```{r, cache = TRUE, fig.cap="The number of necessary trials is plotted against the parameter value."}
...
$$
Since $\beta$ is an inverse scale parameter, we can sample from $\mathrm{Gamma}(\alpha, 1)$ and then divide the result by $\beta$, in stead of sampling directly from the distribution. We have created algorithms which generates samples from the gamma distribution for $\alpha < 1$ (rejection sampling) and for $\alpha > 1$ (ratio of uniforms). When $X \sim \mathrm{Gamma}(1, \beta)$, i.e $\alpha = 1$, $X \sim \mathrm{Exp}(\beta)$, so we can simply use the algorithm we implemented for the exponential distribution. The algorithm which generates samples from $\mathrm{Gamma}(\alpha, \beta)$ is given below.
...
sample.gamma <- function(n, alpha, beta){
  if(alpha == 1){
    ...
    return(gamma.rou(n, alpha)$sim/beta)
  } else{
...
Since we have tested each of the functions which the above algorithm is comprised of, we do not conduct any extensive testing of it here, but we check that it gives sensible results for one combination of parameters, $\alpha = \beta = 3$, shown in figure 15. 
```{r, cache=TRUE, fig.cap="The theoretical density of the gamma distribution plotted with the distribution of our generated samples  with the generic sampling function."}
n <- 10000
alpha <-  3
beta <- 3
...
### (a)
We repeat the problem description for completeness. Let $X \sim \text{Gamma}(\alpha, 1)$ and  $Y \sim \text{Gamma}(\beta, 1)$. Since $X$ and $Y$ are independent, their joint density is $f_{X,Y}(x,y) = f_X(x)f_Y(y)$. Next, define $Z = \frac{X}{X + Y}$ and $V = X + Y$. Then $X = ZV$ and $Y=V(1-Z)$. By the transformation formula, the joint density of $Z$ and $V$ is
...
sim.beta <- function(n, alpha, beta){
  ...
  x <- sample.gamma(n, alpha, 1)
  y <- sample.gamma(n, beta, 1)
...
We check that the distribution of the samples matches the theoretical density for $\alpha = 2$ and $\beta = 10$. This is done in figure 16.
```{r, fig.cap=" The theoretical density of the beta distribution plotted with the distribution of our generated samples."}
n <- 100000
alpha <-  2
beta <- 10

sim.beta <- sim.beta(n, alpha, beta)
x = seq(from = 0.01, to = 1, by = 0.01)
exact <- data.frame(x = x, y = gamma(alpha + beta)/(gamma(alpha)*gamma(beta)) * x^(alpha - 1) * (1 - x)^(beta - 1))

ggplot(data.frame(x = sim.beta)) + 
geom_histogram(aes(x = x, y = ..density.., fill = "Simulated"),
  ...
  scale_fill_manual(name = "", values = c("Simulated" = "#69b3a2"))
```
We also compare the sample mean and variance to the theoretical values.
```{r}
c("mean.theoretical" = alpha/(alpha + beta), "mean.sample" = mean(sim.beta))
c("var.theoretical" = alpha*beta/((alpha + beta)^2*(alpha + beta + 1)),
  "var.sample" = var(sim.beta))
```
...
# Problem C
## C.1
Let $\mathbb{I}$ denote the indicator function. We observe that 
$$
...
$$
1 = \int_4^{\infty}g(x)dx = -c\exp\left(-\frac{x^2}{2}\right)\Bigg|_4^{\infty} \iff c= e^8.
$$
The cumulative distribution, $G(x)$, is thus given as
$$
G(x) = \int_4^{x}g(t)dt = 1 - \exp\left(-\frac{x^2}{2} - 8\right).
$$
It can easily be shown that $G^{-1}(u) = \sqrt{16-2\ln(1-u)}$, which we utilize in the inversion sampling algorithm below. We also conduct our usual sanity check by plotting the distribution of the samples together with the theoretical distribution in figure 17.
```{r,  fig.cap="The theoretical density of g plotted with the distribution of our generated samples."}
inv.samp <- function(n){
  u <- runif(n, 0, 1)
  x <- sqrt(16 - 2*log(1 - u))
  return(x)
...
ggplot(data.frame(x = sim)) + 
 geom_histogram(aes(x = x, y = ..density.., fill = "Simulated"),
  ...
  scale_fill_manual(name = "", values = c("Simulated" = "#69b3a2"))
```
Then, in the importance sampling scheme, the estimator is given as
$$
...
\end{split}
$$
where we have used the sample variances computed in the code above to create the estimate of $m$.
...
$$
\hat\theta_{IS}^{(1)} = \frac{1}{n} \sum_{i = 1}^nh\left(X_i^{(1)}\right)w\left(X_i^{(1)}\right) \;\text{and}\quad \hat\theta_{IS}^{(2)} =  \frac{1}{n} \sum_{i = 1}^nh\left(X_i^{(2)}\right)w\left(X_i^{(2)}\right)
$$
...
```
### (b)
Below, we generate $n = 50000$ pairs of antithetic variates which we use in the importance sampling. We choose $n$ as such because we generate pairs and $2n = 100000$, which is what we used in C.2. An estimate of the antithetic sampler is given below.
```{r}
n <- 50000
c <- exp(8)
...
$$
We assume these $Y_i$'s to be independent, so that we can utilize the $t$-distribution when estimating the variance. We also observe that the variance of the antithetic sampler is:
$$
\begin{split}
...
```{r}
Y <- 1/2 * (h.1*w.1 + h.2*w.2) # Vector of Yi's
S2.Y <- var(Y) # Sample variance of Y's
sigma.as <- sqrt(S2.Y/n) # Estimated sd of theta.as
...
We observe that the precision is higher than for regular importance sampling. This can be explained by a negative correlation coefficient, $\rho$. To check this, we calculate the corresponding sample correlation:
...
## D.1
We consider a multinomial distribution with counts $y=\begin{bmatrix}y_1 & y_2 & y_3 & y_4\end{bmatrix}^T$ and observed data $y = \begin{bmatrix}125 & 18 & 20 & 34\end{bmatrix}^T$. The mass function is given as
$$
f(y \mid \theta) \propto (2+\theta)^{y_1}(1-\theta)^{y_2+y_3}\theta^{y_4},
$$
where $\theta \in (0,1)$. Using a prior $g(\theta)$ on $(0,1)$, the observed posterior density is $f(\theta\mid y) \propto f(y \mid\theta)\ g(\theta)$. First, let $g(\theta) = \mathcal{U}_{[0,1]}$, i.e. the uniform distribution, which is equivalent to $\text{Beta}(1,1)$. Then the observed posterior density becomes
$$
...
$$
We can now use rejection sampling to sample from $f(\theta \mid y)$ using the improper density $\tilde f(\theta\mid y)$ defined in the eauation above. For the rejection sampling to work optimally, meaning we want highest possible acceptance rate, we want to scale the density such that $\text{max}_\theta\ \frac{\tilde f(\theta \mid y)}{\tilde c\ g(\theta)} = 1$, where $\tilde c$ is a constant. When $g(\theta) = \mathcal{U}_{[0,1]}$, the maximum of $\tilde f(\theta\mid y)$ can be found from maximizing its logarithm. Deriving $\ln \tilde f(\theta\mid y)$ with respect to $\theta$ and setting equal to zero, we obtain
$$
...
$$
where we used the values for $\{y_i\}_{i=1}^4$ given in the problem description. Next we use this to find the constant $\tilde c = \tilde f(\theta^*\mid y) \approx \exp(67.38)$. In effect, this means that $\max_{\theta \in (0,1)}\frac{1}{\tilde c} \tilde f(\theta \mid y) = 1$ and $\tilde f(\theta\mid y) \ge 0$ for $\theta \in (0,1)$. Our implementation of the rejection algorithm is summarized below:
...
$$
where $\{\theta_i\}_{i=1}^M$ are sampled from the posterior density $f(\theta\mid y)$. The posterior mean is estimated with $M = 10000$.
```{r}
...
Using $M=10000$ samples, Monte Carlo integration gives the estimate $\hat \mu_{\theta} =$ `r mu.theta`. Next we perform our usual sanity check and draw a vertical line indicating the mean, shown in figure 18.

```{r, fig.cap="The posterior density of the parameter in the multinomial distribution plotted with our generated samples. The vertical black line corresponds to the Monte Carlo estimated mean."}
...
In addition to the practical approach, a numerical estimate can be carried out by calculating the expected acceptance rate. Let the acceptance rate be $\alpha(\theta) = \frac{\tilde f(\theta\mid y)}{\tilde c\ g(\theta)}$. Then the expected value is
$$
\text{E}(\alpha) := \int_0^1 \alpha(\theta)\ g(\theta)\ \text{d}\theta = \frac{1}{\tilde c} \int_0^1 \tilde f(\theta)\ \text{d}\theta,
$$
which is computed numerically. Thus the theoretical expected number of generations needed for one sample is $\frac{1}{\text{E}(\alpha)}$.
...
```
We see that the average number of generations needed to produce one sample are calculated practically as `r 1/acc.rate.practical` and numerically as `r 1/acc.rate.numerical`. The practical estimate is close to the numerical value, as expected.
...
$$
where the importance weights are $w(\theta_i) = \frac{f_2(\theta_i\mid y)}{f(\theta_i\mid y)} = 5(1-\theta_i)^4$. The new posterior mean is calculated below:
```{r}
...
mu.theta2
```
We see that when using the $\text{Beta}(1,5)$ prior, our estimate for the posterior mean $\hat \mu_{\theta,2} =$ `r mu.theta2` is lower than the estimated posterior mean using the uniform prior. The reason for this is that the densities in $\text{Beta}(1,5)$ is higher in the lower end of the interval $(0,1)$, such that samples with lower values of $\theta$ has higher importance weights and thus contributes more relative to samples with higher values of $\theta$. Intuitively, this in turn means that the estimate of the mean must be lower using this prior.
