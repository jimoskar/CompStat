  ...
  if(plot){
    plot <- ggplot(gg.df, aes(x = x)) + geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
      ...
      scale_color_manual(name = " ", values = c("yt/nt" = "blue", "MCMC-predictions" = "black"),
                          ...
                          = c(expression(y[t]/n[t]), "MCMC-predictions")) +
      theme_minimal()
    return(plot)
  } else{
    return(gg.df)
...
set.seed(4300)
ptm <- proc.time()
mcmc.ss <- mcmc.single(num.iter, sigma0 =  0.02, tau0 = rnorm(T))
...
# Acceptance rate
acceptance.rate <- mcmc.ss$count/((num.iter-1) * T)
...
mcmc.data.all <- data.frame("x"         = 1:num.iter,
                        "sigma.vec" = mcmc.ss$sigma.vec,
                        "pi_1"     = sigm(mcmc.ss$tau.mat[,1]),
                        "pi_201"   = sigm(mcmc.ss$tau.mat[,201]),
                        "pi_366"   = sigm(mcmc.ss$tau.mat[,366])
)
...
mcmc.corr <- data.frame("lag"       = 0:max.lag,
                        "sigma.vec" = sacf(mcmc.ss$sigma.vec, max.lag)$rho.hat,
                        "pi_1"     = sacf(sigm(mcmc.ss$tau.mat[,1]), max.lag)$rho.hat,
                        "pi_201"   = sacf(sigm(mcmc.ss$tau.mat[,201]), max.lag)$rho.hat,
                        "pi_366"   = sacf(sigm(mcmc.ss$tau.mat[,366]), max.lag)$rho.hat
                        )
