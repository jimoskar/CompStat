...
$$
f(x) = \frac{e^{\alpha x}}{(1 + e^{\alpha x})^2}, \quad x\in(-\infty, \infty), \;\alpha >0.
$$
...
plot(y[1,1:500], y[2,1:500], main="Bivariate normal distribution", xlab="x", ylab="y", pch=16, cex = 0.4)
...
## B.1
### (a)
We consider the gamma distribution with $\alpha \in (0,1)$ and $\beta = 0$, i.e.
$$
f(x) = \begin{cases} \frac{1}{\Gamma(\alpha)}x^{\alpha -1}e^{-x}, \quad &0 < x, \\
0, \quad &\text{otherwise}.
\end{cases}
$$
Let $g$ denote the distribution considered in A.2. Then, for $x\in(0,1)$, 
$$
\frac{g(x)}{f(x)} = \frac{\Gamma(\alpha)x^{\alpha-1}}{\left(\frac{1}{\alpha} + \frac{1}{e}\right)x^{\alpha-1}e^{-x}} = \frac{\Gamma(\alpha)}{\left(\frac{1}{\alpha} + \frac{1}{e}\right)}e^x \ge \frac{\Gamma(\alpha)}{\left(\frac{1}{\alpha} + \frac{1}{e}\right)} =: k.
$$
For $x \ge 1$, we have
$$
\frac{g(x)}{f(x)} = \frac{\Gamma(\alpha)e^{-x}}{\left(\frac{1}{\alpha} + \frac{1}{e}\right)x^{\alpha-1}e^{-x}} = \frac{\Gamma(\alpha)}{\left(\frac{1}{\alpha} + \frac{1}{e}\right)}x^{1-\alpha} \ge \frac{\Gamma(\alpha)}{\left(\frac{1}{\alpha} + \frac{1}{e}\right)} = k.
$$
Consequently, we know that $f(x) \le \frac{1}{k} g(x) \;\forall x$, and the acceptance probablity is given by

$$
\xi(x) = k\frac{f(x)}{g(x)} = \begin{cases} e^{-x}, \quad &0<x<1,\\
x^{\alpha-1}, \quad &x \ge 1. 
\end{cases}
$$
...
```{r}
rej.samp <- function(n, alpha){
  count <- 0
  result <- rep(NA, n)
  while(count < n){
    x <- sim.g(alpha, 1)
    u <- runif(1,0,1)
    a <- NA # acceptance probability
...
sim.gamma.rej <- data.frame(x = rej.samp(n, alpha))
...
```{r}
samp.log.unif <- function(n, a, b){
  u <- runif(n)
  x <- log((b-a)*u + a)
  return(x)
}
rou.gamma <- function(n, alpha){
  #log.a <-  (1-alpha)/2 * (log(alpha - 1) -1)
  #log.b.plus <- (alpha + 1)/2 * (log(alpha + 1) - 1)
  a <- ((alpha - 1)/exp(1))^((1 - alpha)/2)
  ...
  while(count < n){
    log.x1 <- samp.log.unif(1, 0, a)
    log.x2 <- samp.log.unif(1, b.minus, b.plus)
    print(log.x1 )
    print(0.5*((alpha - 1)*(log.x2 - log.x1) - exp(log.x2 - log.x1)))
    
  
    if(log.x1 <= 0.5*((alpha - 1)*(log.x2 - log.x1) - exp(log.x2 - log.x1))){
...
x = seq(from = 0.01, to = 12, by = 0.1)
exact <- data.frame(x = x, y = 1/gamma(alpha) * x^(alpha - 1) * exp(-x))

ggplot(data.frame(x = sim.gamma$sim)) + 
geom_histogram(aes(x = x, y = ..density.., fill = "Simulated"),
                 alpha = 0.8, binwidth = 0.07) + 
  geom_line(data = exact , aes(x = x, y = y, color = 'Exact'))  +
  scale_color_manual(name = "", values = c("Exact" = "#e0474c")) +
  scale_fill_manual(name = "", values = c("Simulated" = "#69b3a2"))
...
m <- 10 # length of alpha.vec
alpha.vec <- seq(from = 1.01, to = 10, length.out = m)
tries.vec <- rep(0, m)
for(i in 1:m){
  alpha <- alpha.vec[i]
  sim.gamma <- rou.gamma(n, alpha)
