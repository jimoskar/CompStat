...
Judging from the printout above, we conclude that the ratio of uniforms algorithm works correctly. Next we plot the necessary tries to sample $n=1000$ samples with this algorithm for $\alpha \in (0, 2000]$.
...
Since we have tested each of the functions which the above algorithm is comprised of, we do not conduct any extensive testing of it here, but we check that it gives sensible results for one combination of parameters.
```{r, cache=TRUE}
n <- 10000
alpha <-  3
beta <- 3
...
We check that the distribution of the samples matches the theoretical distribution for $\alpha = 2$ and $\beta = 10$.
```{r}
n <- 100000
alpha <-  2
beta <- 10
...
$$
It can easily be shown that $G^{-1}(u) = \sqrt{16-2\ln(1-u)}$, which we utilize in the inversion sampling algorithm below. We also conduct our usual sanity check by plotting the distribution of the samples together with the theoretical distribution.
```{r}
inv.samp <- function(n){
  u <- runif(n, 0, 1)
  x <- sqrt(16 - 2*log(1 - u))
...
Using $M=10000$ samples, Monte-Carlo integration gives the estimate $\hat \mu_{\theta} =$ `r mu.theta`. Next we perform our usual sanity check and draws a vertical line indicating the mean.
