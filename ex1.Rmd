...
# Problem C
## C.1
We observe that $\theta = \mathrm{Pr}\{X > 4\} = \int_4^{\infty}f_X(x) = \int_{-\infty}^{\infty}\mathbb{I}(x > 4) f_X(x) = \mathrm{E}[\mathbb{I}(X >4)].$. Then, by the method of Monte Carlo (MC) integration, an estimate of $\theta$ is
...
$$
\begin{split}
\mathrm{Var}[\hat\theta] &= \frac{1}{n^2}\sum_{i = 1}^n\mathrm{Var}[h(X_i)] = \frac{1}{n}\mathrm{Var}[X] \\
