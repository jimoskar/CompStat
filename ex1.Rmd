...
# Problem C
## C.1
We observe that $\theta = \mathrm{Pr}\{X > 4\} = \int_4^{\infty}f_X(x) = \int_{-\infty}^{\infty}\mathbb{I}(x > 4) f_X(x)$. Then, by the method of Monte Carlo (MC) integration, an estimate of $\theta$ is

$$
\hat{\theta} = \frac{1}{n}\sum_{i = 1}^n\mathbb{I}(X > 4).
$$
Here, $n$ is the number of samples, $\mathbb{I}$ is the indicator function and $X \sim \mathcal{N}(0, 1)$.
